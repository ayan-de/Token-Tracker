...
pub fn start_backend(app: &mut tauri::App) -> Result<(), String> {
    ...
    while start.elapsed() < timeout {
        if TcpStream::connect("127.0.0.1:46727").is_ok() {
            healthy = true;
            break;
        }
    ...
    if !healthy {
        let mut child = child;
        let _ = child.kill();
        return Err("Backend failed to respond on port 46727 within 5 seconds".to_string());
    }

    println!("Backend is healthy and listening on port 46727");
